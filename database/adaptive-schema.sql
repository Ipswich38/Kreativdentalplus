...
BEGIN
  ...
  -- Staff trigger
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staff' AND column_name = 'updated_at') THEN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'appointments' AND column_name = 'updated_at') THEN
...
DO $$
BEGIN
  -- Basic policies that should work with any table structure
  ...
  -- Only create admin policy if profiles table exists with role column
...
-- Insert services only if the table has the required structure
DO $$
BEGIN
