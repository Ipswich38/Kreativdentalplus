-- STEP 5: DIAGNOSE EXISTING TABLES AND FIX ALL ISSUES
-- This script will check what exists and fix everything properly
...
-- Check columns in services table (if exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'services') THEN
        RAISE NOTICE 'SERVICES TABLE COLUMNS:';
        FOR r IN (SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'services' ORDER BY ordinal_position)
        LOOP
...
-- Check columns in inventory table (if exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'inventory') THEN
        RAISE NOTICE 'INVENTORY TABLE COLUMNS:';
        FOR r IN (SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'inventory' ORDER BY ordinal_position)
        LOOP
            RAISE NOTICE '  %: %', r.column_name, r.data_type;
        END LOOP;
    ELSE
        RAISE NOTICE 'Inventory table does not exist';
    END IF;
END $$;

-- Drop all problematic tables and recreate fresh
...
-- Recreate financial transactions table
...
-- NOW INSERT DATA WITH CORRECT COLUMNS
