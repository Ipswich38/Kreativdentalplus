-- STEP 3: SAFE TABLE CREATION AND UPDATES
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='services' AND column_name='service_code') THEN
        ALTER TABLE services ADD COLUMN service_code VARCHAR(20);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='services' AND column_name='requires_dentist') THEN
        ALTER TABLE services ADD COLUMN requires_dentist BOOLEAN DEFAULT true;
    END IF;
...
-- 2. SERVICES TABLE (if not exists)
...
-- Clear existing services data and insert fresh sample data
TRUNCATE TABLE services CASCADE;

-- INSERT SAMPLE SERVICES (safe insert)
...
-- Clear existing patients data and insert fresh sample data
...
-- Clear existing inventory data and insert fresh sample data
